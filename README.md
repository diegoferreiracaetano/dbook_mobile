...
- **M15 — Minhas Viagens persistidas, Próximas/Anteriores**: completo. Antes era só memória de sessão (`BookingRecord`) — sumia ao reabrir o app. Agora `MyBookingsPage` busca de verdade via `GET /bookings` (M17 do backend `dbook`), separa em "Próximas"/"Anteriores" (filtro client-side pela data real do voo) e cada card mostra a foto do destino (cruzada com a mesma lista já carregada pela Home/Explore, sem fetch novo), companhia e modelo do avião reais. Reservar invalida a lista automaticamente; cancelar re-busca do backend.
- **M16 — Resultados de busca mais ricos**: completo. `DbookFlightResultTile` mostra o `aircraftType` real do voo (M16 do backend `dbook`) numa linha extra; `scripts/seed-flights.sh` concentra a janela de datas geradas em 21 dias (era 60), quase triplicando a densidade de voos por rota+data sem mudar rotas/companhias sorteadas. Último marco do plano de redesign da parte logada aprovado em 2026-09-14 (Perfil real, assento por modelo de avião, Minhas Viagens persistidas, resultados mais ricos).
- **M17 — Round Trip real + Anteriores por status**: completo. "Round Trip" buscava e mostrava um campo "Return", mas nunca reservava a volta — o campo era só cosmético. Agora a busca sempre devolve os dois trechos (ida + volta, datas/rota corretas). A jornada escolhe primeiro o voo de CADA trecho (ida, depois volta) e só então começa a seleção de assento, um trecho de cada vez — reservar o assento da ida oferece "Choose Seat" direto pro assento da volta (o voo já foi escolhido antes, não busca de novo), fechando com uma segunda reserva real e independente. De quebra, corrigiu um overflow de layout na tela de sucesso quando o rótulo do botão fica mais longo, e um bug latente de re-pedir login no trecho seguinte de um convidado. Minhas Viagens também passou a considerar o status: uma reserva cancelada vai para "Anteriores" mesmo com o voo ainda no futuro.
- **M18 — Assento vira detalhe, revisão + pagamento no final**: completo. Escolher o assento de cada trecho deixou de abrir uma tela de sucesso própria — `SeatSelectionPage` agora só avisa quem a montou (`onBooked`), que encadeia silenciosamente pro próximo trecho (sem tela intermediária) e, no último, leva pra `PaymentPage`: uma única revisão + pagamento cobrindo TODOS os trechos reservados de uma vez (o assento de cada um vira só uma linha de detalhe no Resumo do Pedido, com o preço real de `flight.price`, já em memória — sem taxa/imposto fictício nem checkbox de "salvar cartão", nenhum dos dois tem dado ou função real por trás). Pagar chama `POST /payments` (M18 do backend `dbook`) com só os 4 últimos dígitos do cartão e o nome do titular — número completo e CVV nunca saem do dispositivo, já que não existe gateway de pagamento real por trás. `BookingSuccessPage` (uma confirmação por trecho) foi substituída por `PaymentSuccessPage` (uma confirmação só, no final, listando todos os trechos pagos). Addendum: `FlightDetailPage` ganhou cabeçalho em foto cheia do destino (cruzada com o mesmo catálogo já carregado, sem fetch novo) e selo da companhia aérea; os campos do cartão em `PaymentPage` ganharam `hintText` de exemplo pra facilitar teste manual.
- Em andamento — M9 (parcial); M10-M18 completos.
