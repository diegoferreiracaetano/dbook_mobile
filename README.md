...
- **M12 — Explore com "Principais destinos" e "Destinos por região"**: completo. A aba Explore separa os destinos em dois grupos em vez de uma grade única: "Principais destinos" filtra `destination.isPopular` (curadoria real do backend, M14 do `dbook`) e "Destinos por região" agrupa a mesma lista por `destination.region`. Uma chamada só (`GET /destinations`, sem mudança de endpoint) — os dois grupos vêm da mesma lista já carregada por `featuredDestinationsProvider`, sem provider novo nem estrutura paralela pra "regiões".
