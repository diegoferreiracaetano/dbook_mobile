...
- **M17 — Round Trip real + Anteriores por status**: completo. "Round Trip" buscava e mostrava um campo "Return", mas nunca reservava a volta — o campo era só cosmético. Agora a busca sempre devolve os dois trechos (ida + volta, datas/rota corretas), reaproveitando a mesma fila de "próximo trecho" já usada pelo Multi-city: reservar a ida oferece buscar e reservar a volta como uma segunda reserva real e independente. De quebra, corrigiu um overflow de layout na tela de sucesso quando o rótulo do botão fica mais longo. Minhas Viagens também passou a considerar o status: uma reserva cancelada vai para "Anteriores" mesmo com o voo ainda no futuro.
