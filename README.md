...
`dbook_domain`/`dbook_core_network` (entidades/DTOs) e `dbook_feature_auth`/`dbook_feature_flights` (estado, via `freezed` puro sem `json_serializable`) usam codegen; depois de mexer nesses arquivos, rodar `dart run build_runner build --delete-conflicting-outputs` dentro do pacote — os `.freezed.dart`/`.g.dart` ficam versionados (sem passo de codegen no CI).
...
- **M5 em diante**: ainda não iniciado.
