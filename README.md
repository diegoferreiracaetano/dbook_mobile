...
- **M12 — "Principais destinos" e "Destinos por região" (Explore + Home)**: completo. Home e Explore separam os destinos em dois grupos em vez de uma grade/lista única: "Destinos em destaque"/"Principais destinos" filtra `destination.isPopular` (curadoria real do backend, M14 do `dbook`) e "Destinos por região" (`DestinationsByRegion`, widget compartilhado pelas duas telas) é um filtro de verdade — chips de seleção única ("Todos" + uma por região) acima de uma grade que mostra só os destinos da região escolhida, não seções estáticas. Uma chamada só (`GET /destinations`, sem mudança de endpoint) — os grupos vêm da mesma lista já carregada por `featuredDestinationsProvider`, sem provider novo nem estrutura paralela pra "regiões". `DbookChipRow` (faixa de chips, mesmo padrão visual do `DbookFareDateStrip`) entrou no design system antes da feature usar.
- Em andamento — M9 (parcial); M10, M11 e M12 completos.
