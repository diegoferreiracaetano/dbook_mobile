...
- **M12 — "Principais destinos" e "Destinos por região" (Explore + Home)**: completo. Home e Explore separam os destinos em dois grupos em vez de uma grade/lista única: "Destinos em destaque"/"Principais destinos" filtra `destination.isPopular` (curadoria real do backend, M14 do `dbook`) e "Destinos por região" (`DestinationsByRegion`, widget compartilhado pelas duas telas) agrupa a mesma lista por `destination.region`. Uma chamada só (`GET /destinations`, sem mudança de endpoint) — os grupos vêm da mesma lista já carregada por `featuredDestinationsProvider`, sem provider novo nem estrutura paralela pra "regiões".
